        ...
        try {
            ...
            for (const gameDoc of gamesSnapshot.docs) {
                ...
                if (turnDuration > AFK_TIMEOUT_MS) {
                    ...
                    if (currentPlayer) {
                        log.info(`AFK detected: ${currentPlayer.displayName} in game ${gameDoc.id}`);
            ...
            if (batchCount > 0) {
                await batch.commit();
                log.info(`Processed ${batchCount} AFK players`);
            }

            return null;
        } catch (error) {
            log.error('Error checking AFK players:', error);
            return null;
        }
    });
...
exports.cleanupOldRooms = functions.pubsub
    ...
    .onRun(async (context) => {
        ...
        try {
            ...
            for (const roomDoc of staleRoomsSnapshot.docs) {
                ...
                if (room.status === 'playing') {
                    ...
                    if (!gameSnapshot.empty) {
                        log.info(`Skipping room ${roomDoc.id} - has active game`);
                        continue;
                    }
                }
                
                log.info(`Deleting stale room: ${roomDoc.id} (last updated: ${room.lastUpdated?.toDate()})`);
            ...
            if (deleteCount > 0) {
                await batch.commit();
                log.info(`Deleted ${deleteCount} stale rooms`);
            }

            return null;
        } catch (error) {
            log.error('Error cleaning up rooms:', error);
            return null;
        }
    });
    ...
    .onDelete(async (snapshot, context) => {
        ...
        try {
            ...
            if (!gamesSnapshot.empty) {
                ...
                await batch.commit();
                log.info(`Cleaned up ${gamesSnapshot.size} games for room ${roomId}`);
            }
            
            return null;
        } catch (error) {
            log.error('Error cleaning up room data:', error);
            return null;
        }
    });
...
exports.logGameAction = functions.firestore
    ...
    .onCreate(async (snapshot, context) => {
        ...
        try {
            ...
            log.info(`Logged action: ${action.type} in game ${gameId}`);
            return null;
        } catch (error) {
            log.error('Error logging game action:', error);
            return null;
        }
    });
...
exports.onGameEnded = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        try {
            ...
            await batch.commit();
            log.info(`Updated stats for game ${gameId}`);
            
            return null;
        } catch (error) {
            log.error('Error updating game stats:', error);
            return null;
        }
    });
...
exports.forceEndGame = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        log.error('Error forcing game end:', error);
